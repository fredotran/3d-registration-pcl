...
int main(int argc, char **argv)
{
    ...
    std::string configFilenames[] = {"../data/exp_sacia_param_grid_search_yaw110deg.txt"}; // including path
                ...
                for (double saciaNumSample : saciaNumSampleVals)
                {
                    ...
                    for (double saciaMinSampleDist : saciaMinSampleDistVals)
                    {
                        ...
                        for (double saciaMaxCorrDist : saciaMaxCorrDistVals)
                        {
                            ...
                            {
                                pipelineSettings.setValue(NORMALS_SEARCH_RADIUS, normalsSearchRadius);

                                for (double fpfhSearchRadius : fpfhSearchRadiusVals)
                                {
                                    pipelineSettings.setValue(FPFH_SEARCH_RADIUS, fpfhSearchRadius*normalsSearchRadius);
