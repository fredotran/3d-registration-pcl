...
/* Generates a random number between min and max (uniform distribution). The seed will be
incremented so that it can be directly used in subsequent calls. */
double randomizeDoubleUniform(double* seed, double min, double max) {
    *seed += 1;
...
/* Randomize a 2D coordinate within the provided boundaries under the condition that the distance
between the coordinate and the boundaries may not be less than the specified buffers. */
std::array<double, 2> randomizeCoordinate(double* seed, double boundariesX[2],
                                          double boundariesY[2], double bufferX, double bufferY) {
...
/* Returns randomized rectangular (xMin, xMax, yMin, yMax) cut-out boundaries to be used for
reference point cloud extraction from surface model point cloud(input: surface model data
boundaries, source_width, source_height, x_uncertainty, y_uncertainty; output: reference PC x/y
limits) */
...
/* Returns randomized rectangular (xMin, xMax, yMin, yMax) cut-out boundaries to be used for source
point cloud extraction from reference point cloud */
std::array<double, 4> randomizeSourceCutout(double* seed, double refModelBoundariesX[2],
                                            double refModelBoundariesY[2], double sourceWidth,
                                            double sourceHeight) {
