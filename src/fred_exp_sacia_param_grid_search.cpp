...
int main(int argc, char** argv) {
    ...
    std::string configFilenames[] = {"../data/exp_sacia_param_grid_search_yaw110deg.txt"};
    ...
    double normalsSearchRadiusVals[] = {1.0, 1.5, 3.0, 6.0, 9.0};
    double fpfhSearchRadiusVals[] = {1.2, 1.5, 2.0, 3.0, 5.0};
    int numberIter = 1;
                ...
                for (double saciaNumSample : saciaNumSampleVals) {
                    ...
                    for (double saciaMinSampleDist : saciaMinSampleDistVals) {
                        ...
                        for (double saciaMaxCorrDist : saciaMaxCorrDistVals) {
                            ...
                            for (double normalsSearchRadius : normalsSearchRadiusVals) {
                                pipelineSettings.setValue(NORMALS_SEARCH_RADIUS,
                                                          normalsSearchRadius);

                                for (double fpfhSearchRadius : fpfhSearchRadiusVals) {
                                    pipelineSettings.setValue(
                                        FPFH_SEARCH_RADIUS, fpfhSearchRadius * normalsSearchRadius);

                                    fullRegistration(configFilename, pipelineSettings,
                                                     outputFilename, &seedRef, &seedSource,
                                                     &seedCustomRotation);
                                }
                            }
                        }
