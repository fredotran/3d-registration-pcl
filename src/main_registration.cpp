...
int main(int argc, char **argv)
{
    ...
    if (detector == "1") detector = "all";
    else if (detector == "2") detector = "sift";
    else if (detector == "3") detector = "harris";
    else if (detector == "4") detector = "iss";

    Settings pipelineSettings = getPipelineDefaultSettings();

    auto runDetector = [&](const std::string& name, const std::string& paramFile, const std::string& resultFile) {
        std::cout << "\n" << name << " RUNS\n" << std::endl;
        for (size_t i = 1; i <= numberIter; i++)
        {
            double seedRef(0.5), seedSource(0.5), seedCustomRotation(0.5);
            fullRegistration(paramFile, pipelineSettings, resultFile,
    ...
    {
        runDetector("HARRIS", "../data/harris_600m-40deg-FOV_rotation-Z.txt", "../results/harris_results");
    }
    if (runAll || detector == "sift")
    {
        runDetector("SIFT", "../data/sift_600m-40deg-FOV_rotation-Z.txt", "../results/sift_results");
    }
