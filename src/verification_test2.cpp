...
// RUN A REGISTRATION BETWEEN 2 POINT CLOUDS SAMPLES (source_verification2.pcd & transformed
// source_verification2.pcd) WITH A CUSTOM TRANSFORMATION Results expected = enough registration to
// be able to perform ICP

int main(int argc, char** argv) {
    time_t timetoday;
    time(&timetoday);
    std::cout << "verification_test2 initiated on : " << asctime(localtime(&timetoday))
              << std::endl;
    ...
    Eigen::Affine3f transform =
        customRotation(sourceCloudPtr, Eigen::Vector4f::Zero(), Eigen::Vector4f::Zero(),
                       Eigen::Vector4f::Zero(), 0.0f, 0.0f, 70.0f);
    ...
    auto pipelineOutputPtr =
        pipelineAllPoints(sourceTransformedCloudPtr, targetCloudPtr, pipelineSettings);
    ...
    pcl::transformPointCloud(*sourceTransformedCloudPtr, *finalTransformedCloudPtr,
